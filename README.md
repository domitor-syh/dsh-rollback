...
| DSH 版本 | 状态 | 说明 |
| --- | --- | --- |
| 0.1.5-rc.2 | **已实测** | 本次发布适配并逐项验证的版本：回退、截断、文件恢复、欢迎页、隐藏、图片回填都在这个版本上端到端跑通 |
| 0.1.1-rc.2 | 按设计支持，**未实测** | 插件为旧契约保留了特性探测回退分支：已退役的 `conversationEvents` 服务、快照上的 `session.chat`、旧拼写的表层 `start`/`end`、`workspaces.openPath`、`addImages`/`pruneImages`。本机没有 0.1.1 的构建，这条路径**没有在真实运行 0.1.1 的机器上跑过**——它是设计上的回退，不是实测结论 |
| 低于 0.1.1，或 0.2.0 及以上 | 不支持 | 低于 0.1.1 没有上述回退所覆盖的契约；0.2.0 起契约可能变化，未做适配 |
