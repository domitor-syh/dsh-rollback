...
### 兼容性声明
- **`dsh.engines.dsh` 去掉虚构的上界**：由 `>=0.1.5-rc.2 <0.2.0` 改为 `>=0.1.5-rc.2`。npm 上从未发布过任何 0.2.0 构建——已发布的 DSH 构建止于 0.1.7 的预发布版——写死 `<0.2.0` 等于声明一条并不存在的约束；开区间下界也是生态里的通行写法（如同类插件 `dsh-claude-style` 的 `>=0.1.5-rc.2`）。两个 README 中引用该字段的句子同步改写，逐版本矩阵仍是权威说明。
- 本次发布**只改声明与文档**：代码、行为、依赖与 `dsh.client.inject` 均未变动，bundle 内容与 0.3.0 一致。
