...
- **Boundary re-scan with last known content**: capture only ever sees the file tools, so the plugin re-checks the paths it has watched at **every user-message boundary** (a stat-only fast path while the fingerprint is unchanged) and records what a shell command rewrote or removed **against the turn that boundary opened** — rolling back to before that turn restores it. The decisions fail closed: a file that vanished before the plugin ever read it is **not recorded** (recording it would surface only as an unrestorable path, and any unrestorable file aborts the whole rollback) but warned about once; a file above 8 MiB stops being watched instead of pretending to be restorable; and after a rollback the registry drops only what it learned about the contents, **keeping the paths watched** so coverage is not quietly lost.
...
- **Regression tests**: `tests/truncation-plan.test.ts` (10), `tests/core.test.ts` (28), `tests/root-write.test.ts` (14), `tests/root-write-fallback.test.ts` (15), `tests/literal-edit.test.ts` (12), `tests/dir-cleanup.test.ts` (12), `tests/empty-dirs.test.ts` (5), `tests/boundary-scan.test.ts` (10), `tests/boundary-rescan.test.ts` (8), and `tests/boundary-pipeline.test.ts` (3).
